...
def visualize_weights(W_conv1, num_kernels=32, name=1):					
	with tf.name_scope("Visualize_weights_conv"+ str(name)):
...
logfold = outfold + "logs/1/"
modelfold = outfold + "model/"
...
visualize_weights(W_conv1, num_kernels=32)
...
visualize_weights(W_conv2, num_kernels=64)
...
with tf.name_scope("Conv3"):
	...
	with tf.name_scope("Pool2"):
		...
		#save output of Conv layer to TB- first 16 filters
		with tf.name_scope("Image_conv2"):
			image = A3_conv[0:1, :, :, 0:16]
			image = tf.transpose(image, perm=[3,1,2,0])
			tf.summary.image("image_conv2", image)

visualize_weights(W_conv3, num_kernels=128)
#-----------------------------------------------------------------------
#	Flatten
#-----------------------------------------------------------------------
A2_flatten = flatten(A3_pool)
...
#create directories
pathlib.Path(logfold + '/train/').mkdir(parents=True, exist_ok= True)
pathlib.Path(logfold + '/test/').mkdir(parents=True, exist_ok= True)

#log writers
train_writer = tf.summary.FileWriter(logfold + '/train/', sess.graph)
test_writer = tf.summary.FileWriter(logfold + '/test/')
